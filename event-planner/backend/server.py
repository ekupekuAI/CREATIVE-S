...
try:
    ...
    # Serve workspace root index without shadowing API routes
    @app.get('/', response_class=HTMLResponse)
    async def root_index_page():
        try:
            index_path = workspace_root / 'index.html'
            if index_path.exists():
                return HTMLResponse(content=index_path.read_text(encoding='utf-8'), status_code=200)
        except Exception as _e:
            print('Warning: failed to serve root index:', _e)
        return HTMLResponse(content='<h1>Workspace</h1><p>No index.html found.</p>', status_code=200)
except Exception as e:
...
async def list_events():
    events = load_events()
    # return list of events (id + basics), as an array for frontend
    out = []
    for eid, ev in events.items():
        out.append({
            'id': eid,
            'basics': ev.get('basics', {}),
...
# Provide a root-level health endpoint for convenience
@app.get("/health")
async def root_health():
    return {"status": "healthy", "timestamp": datetime.now().isoformat()}

# Include the event planner API router without prefix; the unified root server
# mounts this sub-app at '/api/event-planner', so the final paths will be
# '/api/event-planner/...'. Using a prefix here would double it and break routes.
app.include_router(event_planner_api)

# ----------------- Mindmap API -----------------
mindmap_api = APIRouter()

class MindmapClassifyRequest(BaseModel):
    text: str

@mindmap_api.get('/health')
async def mindmap_health():
    return {"status": "healthy", "timestamp": datetime.now().isoformat()}

@mindmap_api.post('/classify')
async def mindmap_classify(req: MindmapClassifyRequest):
    if not req.text or not req.text.strip():
        raise HTTPException(status_code=400, detail='text is required')
    # If OpenAI is available, do a simple topic extraction; else return a stub
    if openai_client:
        try:
            messages = [
                {"role": "system", "content": "Extract concise topics from the user text. Return JSON: {\"topics\": [..]}"},
                {"role": "user", "content": req.text}
            ]
            result = try_openai_json(messages) or {"topics": []}
            return {"success": True, "data": result}
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"OpenAI classify failed: {e}")
    # Fallback simple token-based keywords
    tokens = [t.strip('.,;:!?').lower() for t in req.text.split() if len(t) > 4]
    freq: Dict[str, int] = {}
    for t in tokens:
        freq[t] = freq.get(t, 0) + 1
    topics = sorted(freq.keys(), key=lambda k: freq[k], reverse=True)[:10]
    return {"success": True, "data": {"topics": topics}}

app.include_router(mindmap_api, prefix='/api/mindmap')
